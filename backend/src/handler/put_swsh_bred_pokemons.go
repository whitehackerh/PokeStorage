...
func PutSwShBredPokemons(c *gin.Context) {
	var url = "/swsh/bred-pokemons"

	claims := *c.MustGet("claims").(*middleware.Claims)

	db := infrastructure.ConnectDb()
	uc := usecase.NewPutSwShBredPokemonsInteractor(
		...
		presenter.NewPutSwShBredPokemonsPresenter(),
	)

	input := usecase.PutSwShBredPokemonsInput{}
